...
def decantadorSet():
    while True:
        ...
        p = requests.post('http://localhost:5000/Secador/SetDecantador',verify=False,headers={'content-type':'application/json'},data=a)
        s = p.json()
...
def decantadorSee():
    g = requests.get('http://localhost:5000/Secador/SeeDecantador',verify=False)
    s = g.json()
    print(s)
    return s
